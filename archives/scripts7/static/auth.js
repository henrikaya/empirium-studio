...
function setCompleteDivSize(){
	if(_completeDiv){
		_completeDiv.style.left=calculateOffsetLeft(_inputField)+2+"px";
...
function printDiv() {
	...
	/*alert("top: " + _completeDiv.style.top + " / left : " + _completeDiv.style.left + " / width : " + _completeDiv.style.width + " / height : " + _completeDiv.style.height);
	alert("children : " + _completeDiv.childNodes.length);

	if (_completeDiv.childNodes.length > 0) {
		alert("child 0 : " + _completeDiv.childNodes[0].innerHTML);
	}
	*/
}
...
function metsEnPlace(valeur, liste){
	...
	if(liste.length>0) {
		_completeDiv.height=16*liste.length+4;
	} 
	else {
		hideCompleteDiv();
	}
